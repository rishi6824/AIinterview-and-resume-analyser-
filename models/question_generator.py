...
class QuestionGenerator:
    ...
    def generate_questions_raw(self, job_role, resume_analysis, num_questions=3):
        ...
        # If all APIs fail, return minimal fallback
        print("⚠️ All APIs failed, using minimal fallback")
        return [{
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if json_start != -1 and json_end > json_start:
                    obj_text = generated_text[json_start:json_end]
...
Generate exactly {num_questions} conversational questions:"""
    ...
    def generate_questions(self, job_role, resume_analysis, num_questions=None):
        """Generate 10-15 questions using Router API, DeepSeek API, or Hugging Face API"""
        from config import Config
        ...
        print(f"🔄 Starting question generation: {num_questions} questions for {job_role}")
        
        # Try Router API first
        router_questions = self._generate_with_router_api(job_role, resume_analysis, num_questions)
        if router_questions and len(router_questions) >= Config.MIN_QUESTIONS:
            print(f"✅ Successfully generated {len(router_questions)} questions from Router API")
            return router_questions[:num_questions]
        
        # Try DeepSeek API second
        deepseek_questions = self._generate_with_deepseek_api(job_role, resume_analysis, num_questions)
        if deepseek_questions and len(deepseek_questions) >= Config.MIN_QUESTIONS:
            print(f"✅ Successfully generated {len(deepseek_questions)} questions from DeepSeek API")
            return deepseek_questions[:num_questions]
        
        # Try Hugging Face API third
        hf_questions = self._generate_with_hf_api(job_role, resume_analysis, num_questions)
        if hf_questions and len(hf_questions) >= Config.MIN_QUESTIONS:
            print(f"✅ Successfully generated {len(hf_questions)} questions from Hugging Face API")
            return hf_questions[:num_questions]
